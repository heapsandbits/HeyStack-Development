#Program: HayStack - Function Scan
...
for currentLine in fileObject:
    #If a function is found within the current line, add it to the foundFunctionsData list
    if currentLine.find("sub") > 0 and currentLine.find("end") < 0:
        foundFunctions = True

        #Strip new line characters before appending to storage
        currentLine = currentLine.strip()
        foundFunctionsData.append(currentLine)
	
        #Count total functions found
        numberOfLinesWithFunctions = numberOfLinesWithFunctions + 1
	
    #Count total lines scanned
    totalLinesScanned = totalLinesScanned + 1
	
#Determine if functions were found 
if foundFunctions == True:
	print "Found Functions:"
	for currentElement in foundFunctionsData:
		print currentElement
	print ""
	print "Scan Statistics:"
	print numberOfLinesWithFunctions,"/",totalLinesScanned,"lines had functions..."
...
resultsFileName = "function_scan_results-" + fileWriteTimeFormat + ".txt"
...
#strings were found
if foundFunctions == True:
	resultsFileObject.write("Found Functions:\n")
	for currentElement in foundFunctionsData:
		tempFileLine = ""
	...
	tempFileLine = str(numberOfLinesWithFunctions) + "/" \
		+ str(totalLinesScanned) + " lines had functions...\n"
	resultsFileObject.write(tempFileLine)
	tempFileLine = ""
	tempFileLine = "Total Lines Scanned: " + str(totalLinesScanned)
	resultsFileObject.write(tempFileLine)
if foundFunctions == False:
	resultsFileObject.write("No functions were found within the specimen file...")
...
destinationDirectory = "C:\Users\Richard\Desktop\HayStack\Scan Report Files\Function Scans\\" + resultsFileName
